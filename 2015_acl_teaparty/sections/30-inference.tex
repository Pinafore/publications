...
\begin{equation}
  ...
  %\frac{N \subtwo k{w \subtwo dn} \minussuptwo dn + \beta \phi^{\star} \subtwo k{w \subtwo dn}}{N \subtwo k{\cdot} \minussuptwo dn + \beta}
\end{equation}
  ...
  \begin{array}{ll}
    \mathcal{N}(u \subtwo ak; \mu \subthree akj, \rho) \cdot
     ...
     \frac{\lambda}{N \subthree dkj \minussuptwo dn + \lambda} \cdot
        ...
        %\frac{\lambda_0}{\hat{N} \subthree {\cdot}kj \minussuptwo dn + \lambda_0}
        ,
     &
  \end{array}
\right.
...
%We sample the global frame proportion $\hat{\psi}_k$ using the
%\textit{direct assignment} method of \newcite{Teh:JASA06:hdp}.
...
%   &- \frac{1}{2\rho} \sum_{a=1}^A \sum_{k=1}^K \left(u \subtwo ak - \bm \eta_k^T \hat{\bm \psi} \subtwo ak \right)
%   - \frac{1}{2\sigma} \sum_{b=1}^B x_b^2 - \frac{1}{2\sigma} \sum_{b=1}^B y_b^2
% \end{align}


%\paragraph{Practical notes on implementation}
%\label{subsec:c6_implementation_note}
%
%\begin{itemize}
%  \item Initialization
%  \item Threshold
