...
def plot_samples_bar():
    ...
    # g.save(filename=OUTPUTS['preco_samples'], width=7, height=9)
    g.save(filename=OUTPUTS['preco_samples'], width=9, height=12)
...
def plot_preco():
    ...
    g_f1 = make_plot('f1')
    g_f1.save(filename=OUTPUTS['preco_f1'], width=6, height=9)
    # g_f1.save(filename=OUTPUTS['preco_f1'], width=9, height=4)
    g_mention = make_plot('mentions')
    g_mention.save(filename=OUTPUTS['preco_mention'], width=6, height=9)
    return df
...
def plot_userstudy():
    ...
    def plot_sessions(full):
        ...
        if not full:
            ...
            # scores = pd.read_json(datadir('study_scores.jsonl'), lines=True)
            # user_sessions = df.groupby(['user', 'session']).max().reset_index()
            # scores = user_sessions.merge(scores, how='inner', on=['user', 'session'])
            # scores['text_x'] = scores.total_spans + 3
            # scores['text_y'] = scores.total_time.rank() * scores.total_time.max() / 8
            # g += geom_text(aes(x='text_x', y = 'text_y', label='f1'), data=scores, format_string='{:.3f}',
                    # size=13)
        # g.save(filename=gfxdir(f'userstudy{suffix}.pdf'), width=4, height=5)
...
def plot_errors():
    ...
    # df_qbcoref = pd.read_csv(datadir('error_qbcoref.csv'))
    # g_qbcoref = plot_errors_df(df_qbcoref)
    # g_qbcoref.save(filename=OUTPUTS['qbcoref_error'], width=18, height=2)
