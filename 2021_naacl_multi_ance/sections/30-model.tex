...
\jbgcomment{Perhaps I'm missing something, but if we're appending to a
  query vector, doesn't that change the dimensionality?  How do you
  then do MIPS on that?  The composition function seems unclear.}


\czcomment{made the edit (deleted the ``vector''), here we are not appending to a vector, 
but appending the tokens as the new input to BERT,
therefore the dimensionality does not change.}
